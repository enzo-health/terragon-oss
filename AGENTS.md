...
- **Rich Part Renderers**: The live transcript renders through `chat/conversation/` (presentation: `conversation-page.tsx` on the canonical nauval Conversation skeleton, `registry.tsx`, `leaves/`, `chrome/`) with transport hooks in `chat/transcript-data/` — a pure `TranscriptStore` fold (AG-UI events → a closed `TranscriptItem` union) rendered by an exhaustive typed leaf registry (`registry.tsx`) into the vendored nauval components in `components/ai/*`. There is no assistant-ui runtime and no part/tool dispatch table. The legacy `message-part.tsx` / `tool-part.tsx` / `PART_REGISTRY` / `TOOL_DISPATCH` / `ChatMessage` dispatch tree, the AG-UI aggregator island, and the `@assistant-ui/*` runtime + `native-thread` path have all been removed; do not reintroduce them.
...
The chat layer is `@assistant-ui/react`-free. The plan of record is `docs/plans/2026-07-03-simple-architecture.md`: one vocabulary (AG-UI events, directly), one client fold (`TranscriptStore`), one leaf registry. The invariants below are true on `main` — do not regress them. Write routing for existing-thread appends: `POST /api/ag-ui/[threadId]` parses AG-UI command bodies through `handleAgUiPostCommand()`, dispatches appends through `dispatchFollowUpFromAppend()` → `followUpInternal()`, and falls through to the GET SSE path for bodyless, replay, and resume POSTs. Cancel routes through the AG-UI cancel endpoint → `cancelThreadFromAgUiInput()` → `stopThreadInternal()`. AG-UI is transport; Terragon's follow-up/stop pipeline remains the DB writer.

- **Transcript = store fold + typed leaf registry + nauval components.** The live transcript renders via `apps/www/src/components/chat/conversation/` (`conversation-page.tsx` composes the nauval `Conversation` skeleton; chrome — working message, boot checklist, errors, lifecycle rows — lives in `conversation/chrome/` on nauval `Loader`/`Task`/`Callout`/`Exception`/`Status`). `TranscriptStore` (`chat/transcript-store/`) is one pure fold from AG-UI event envelopes to a CLOSED `TranscriptItem` union; `conversation/registry.tsx` maps each `TranscriptItem["kind"]` to exactly one nauval leaf (`conversation/leaves/*`) via a `Record` over the closed union (the compile-time exhaustiveness check). `transcript-data/use-live-transcript.ts` owns transport: it hydrates the store from `?history=messages`, subscribes to the `HttpAgent` event stream, seeds the seq-cursor for replay, opens the resume SSE stream (`agent.runAgent` with an explicit resume intent, gated by `shouldOpenResumeStream`), and classifies typed `RUN_ERROR` into the error banner. Do NOT add a switch/table that dispatches over part or tool names, and do NOT reintroduce an assistant-ui runtime. **Feature recipe — a new renderable agent event costs exactly three edits:** (1) daemon provider adapter emits the `terragon.part` variant, (2) the store fold adds one `TranscriptItem` case, (3) the registry adds one line + leaf. Zero server edits, zero schema migrations. Unknown parts fold to an `unknown-part` item (labeled fallback card), never dropped.
- **Queued user messages render through dedicated leaves.** Queued (not-yet-sent) user parts render via `apps/www/src/components/promptbox/queued-user-parts.tsx` (`QueuedUserPart`), an exhaustive switch over the closed user-part union calling the standalone `TextPart`/`ImagePart`/`RichTextPart`/`PdfPart`/`TextFilePart` renderers directly. Item-level errors (e.g. a Codex `error` item) fold to an `error` `TranscriptItem` and render through the registry into the nauval `Callout`.
- **The composer is the vendored nauval `Composer` → `routeComposerSubmit` → a guarded server-action POST.** `apps/www/src/components/promptbox/simple-promptbox.tsx` renders `components/ai/composer.tsx` (`Composer`/`ComposerToolbar`/`ComposerToolbarSpacer`) with `components/ai/composer-rich.tsx` (`ComposerRichInput` + `ComposerSuggestions`) as the input slot. `ComposerRichInput` is TipTap under the hood, but the input is now a controlled `ComposerValue` (`{text, segments}`) — `use-promptbox.tsx` owns that value and builds the `@`/`/` triggers (async file typeahead for `@`, agent slash commands for `/`) that feed nauval's native suggestion list. The old custom trigger machinery is gone (`folder-aware-mention.ts`, `slash-command-extension.ts`, the tippy popovers — all deleted); do not reintroduce them. **Persisted-schema contract:** `composer-richtext.ts`'s `composerValueToRichText()` must produce the exact `DBUserMessage` rich-text part shape the legacy `tiptap-to-richtext.ts` produced — enforced by the parity test in `composer-richtext.test.ts`. Submission still goes through `routeComposerSubmit()`, which routes every non-queued send through the `handleSubmit` fallback (the `send-message` thread intent → `followUp()` server action, `clientSubmissionId`-idempotent) and active-run sends through `handleQueueMessage` (the `queue-message` intent). The optimistic booting flip is fired inside `routeComposerSubmit` before the fork. Stop routes through the `stop-thread` intent; the client resume stream also closes on the server's terminal event. There is no client runtime and no `useThreadRuntime`; do not reintroduce one. (`ModelSelector`/`ModeSelector` desktop triggers/popovers are nauval `Select` (`components/ai/select.tsx`); the mobile path keeps the shadcn `Drawer` because the multi-agent checkbox flow doesn't map cleanly onto a touch Select.)
- **`@ag-ui/client` and `@ag-ui/core` are pinned to exact versions** (`0.0.52`). These have documented protocol churn between pre-1.0 versions; do not switch either to caret (`^`) or tilde (`~`). Upgrades must be deliberate and tested end-to-end against the integration harness. `@ag-ui/client`'s `HttpAgent` is the transport (SSE + `runAgent`/`abortRun`); the seq-cursor replay stack around it is ours because `HttpAgent` has no native resume/retry.
- **Justified divergence (do NOT propose migrating):** `apps/www/src/components/ai-elements/markdown-renderer.tsx` is a streamdown-based renderer; it exposes `parseIncompleteMarkdown` for streaming-token UX, which off-the-shelf markdown renderers do not.
