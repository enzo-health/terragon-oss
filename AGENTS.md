...
- **Rich Part Renderers**: The live transcript renders through `assistant-ui/native-thread.tsx` plus the nauval components in `components/ai/*`. The legacy `message-part.tsx` / `PART_REGISTRY` / `TOOL_DISPATCH` dispatch tree has been removed; do not reintroduce it.
...
- **Transcript rendering goes through `native-thread` + nauval, not a dispatch table.** The live chat transcript renders via `apps/www/src/components/chat/assistant-ui/native-thread.tsx` and the nauval components under `apps/www/src/components/ai/*`. The former `message-part.tsx` dispatcher, `parts/part-registry.ts` (`PART_REGISTRY`), `tool-part.tsx` (`TOOL_DISPATCH`), and `tools/tool-registry.ts` have been deleted. Read-side `DBPart`/tool typing still flows through `toUIMessages.ts` + `tool-part-projection.ts` (`projectDBToolCall`) into the `UIPartExtended` union; adding a new part/tool variant requires extending that union in `packages/shared/src/db/db-message.ts` and wiring rendering in the native-thread/nauval path. Do not reintroduce the legacy registry/dispatch tables.
...
- **Justified divergences from assistant-ui (do NOT propose migrating these):**
  - `apps/www/src/components/promptbox/use-promptbox.tsx` — TipTap stays as the input slot, but the composer is being wrapped in `ComposerPrimitive.Root` and integrated with the assistant-ui runtime per `docs/plans/2026-04-30-runtime-owns-writes-adr.md`. Submission goes through `useThreadRuntime().append()`, which routes via the AG-UI POST adapter to the `followUp()` server action. Active-run queueing is owned by `routeComposerSubmit()` plus the chat prompt-box queued-message state; do not reintroduce a separate composer queue hook. Slash commands, mentions, drafts, transcription, and attachments stay inside the TipTap slot — `ComposerPrimitive` only owns the `Root`/`Send`/`Cancel`/`AttachmentDropzone` scaffold around it. Do not propose ripping out TipTap; do propose moving any remaining direct `followUp()` calls from the composer onto `runtime.append`.
