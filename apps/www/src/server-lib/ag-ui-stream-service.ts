...
import { getTerragonProps, recordAgentTraceSpan } from "@/lib/agent-trace";
...
// Per-stream memory bound for replay/live-tail deduplication. 2000 is
// ~10x the typical replay tail overlap (200 events) and caps per-stream
// memory at roughly 200 KB (assuming ~100 bytes per serialized key).
const MAX_REPLAY_DEDUPE_KEYS = 2_000;
...
// Re-export for route.ts usage
export type TerragonPostIntent = "append" | "resume";

export function readTerragonPostIntent(
...
export function resolveReplayCursor(request: {
  headers: Headers;
  url: string;
}): ReplayCursor | null {
  ...
  return parseReplayCursor(url.searchParams.get("fromSeq"));
}
...
export type AgUiStreamParams = {
  ...
  dbMessages: readonly DBMessage[];
  /**
   * Pre-resolved effective runId. The route performs eager resolution
   * (including the `getLatestRunIdForThreadChat` fallback) so that mock
   * tests can assert on the DB helpers before the stream body is consumed.
   */
  resolvedRunId: string | null;
  /**
   * Pre-captured Redis stream cursor. The route captures this BEFORE any
   * DB replay query so in-flight XADDs published during the replay window
   * are not dropped by the live tail's `$` cursor.
   */
  initialLastId: string;
};

export type AgUiHistoryResult = {
  messages: DurableAgUiHistoryItem[];
  lastSeq: number;
};

export async function getAgUiHistoryMessages({
  threadChatId,
  dbMessages,
}: {
  threadChatId: string;
  dbMessages: readonly DBMessage[];
}): Promise<AgUiHistoryResult> {
...
export function createAgUiStream(
  ...
  // `resolvedRunId` is mutable inside the stream so that mid-stream
  // synthetic framing (resume RUN_STARTED inference) can update it.
  let resolvedRunId = initialResolvedRunId;
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      requestSignal.addEventListener("abort", () => close("client_abort"), {
        once: true,
      });
      ...
      const boundedDedupeAdd = (key: string) => {
        if (replayedEventDedupeKeys.size >= MAX_REPLAY_DEDUPE_KEYS) {
          const first = replayedEventDedupeKeys.values().next().value as
            | string
            | undefined;
          if (first !== undefined) {
            replayedEventDedupeKeys.delete(first);
          }
        }
        replayedEventDedupeKeys.add(key);
      };
      ...
      if (resolvedRunId === null) {
        ...
        await liveTail(
          resolvedRunId !== null ? { runId: resolvedRunId, userId } : undefined,
        );
        return;
      }
      ...
      if (replayEnvelopes.length === 0) {
        if (
          ...
          await liveTail({ runId: resolvedRunId, userId });
          return;
        }
        if (
          replayCursorSeq !== null &&
