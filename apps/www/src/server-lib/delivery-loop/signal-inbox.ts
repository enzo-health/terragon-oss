...
function classifySignalPolicy(causeType: SdlcLoopCauseType): SignalPolicy {
...
async function claimNextUnprocessedSignal({
  db,
  loopId,
  claimToken,
  now,
...
async function completeSignalClaim(params: {
  db: DB;
  signalId: string;
  claimToken: string;
  now: Date;
