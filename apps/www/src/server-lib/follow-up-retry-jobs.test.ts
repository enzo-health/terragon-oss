...
import {
  drainDueFollowUpRetryJobs,
  getRetryJob,
  scheduleFollowUpRetryJob,
} from "./follow-up-retry-jobs";

describe("follow-up retry jobs", () => {
  beforeAll(() => {
  ...
  async function drainWithRetry(
    args: Parameters<typeof drainDueFollowUpRetryJobs>[0],
  ): ReturnType<typeof drainDueFollowUpRetryJobs> {
    let lastError: unknown;
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        return await drainDueFollowUpRetryJobs(args);
      } catch (error) {
        const message =
