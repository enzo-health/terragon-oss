...
| ---------------------------------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bg/border-surface`                                        | `--color-surface: var(--surface)`                   | the card/panel tone. Registered off the `--surface` base var (default `var(--muted)`) — NOT `--color-*` — so `.nauval-chat-surface` can re-theme it at runtime. `@theme inline` inlines the RHS, so a `--color-surface` override in a scoped block is dead code; utilities read `--surface`. |
| `bg-surface-elevated`                                      | `--color-surface-elevated: var(--surface-elevated)` | raised inset blocks, one elevation step off `--surface`. Same base-var indirection: default `var(--card)` globally, overridden to a darker grey inside `.nauval-chat-surface` for the transcript's grey ladder.                                                                              |
