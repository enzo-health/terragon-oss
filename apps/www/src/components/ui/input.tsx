...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        "file:text-strong placeholder:text-mid selection:bg-coral selection:text-primary-foreground border-hairline flex h-11 w-full min-w-0 rounded-xl border bg-canvas px-5 py-3 text-base text-strong transition-[color,border-color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
