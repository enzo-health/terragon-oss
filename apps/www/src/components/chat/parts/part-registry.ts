/**
 * Typed dispatch table for every `UIPartExtended["type"]`.
 *
 * Each entry adapts the common `PartRegistryContext` plus a narrowed part to
 ...
 * The registry keeps an entry so dispatch stays exhaustive over
 * `UIPartExtended["type"]`.
 */
const NullRenderer: ComponentType<Record<string, never>> = () => null;

/**
 * Inline fallback card for `UIDelegationStubPart`, kept here so the registry
 * remains the single source of truth for part dispatch.
 */
