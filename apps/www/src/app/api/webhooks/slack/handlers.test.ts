...
import { createThread, getThreadChat } from "@terragon/shared/model/threads";
...
describe("handleAppMentionEvent", () => {
  ...
  describe("thread creation parameters", () => {
    ...
    it("routes repeated mentions in the same Slack thread as follow-ups", async () => {
      ...
      await handleAppMentionEvent(
        createBasicEvent({
          text: "<@B123456789> add this to the same task",
      ...
      expect(threadChat?.queuedMessages?.[0]?.parts[0]).toEqual(
        expect.objectContaining({
          type: "text",
          text: expect.stringContaining("add this to the same task"),
        }),
      );
    });

    it("routes legacy Slack threads without teamId as follow-ups", async () => {
      const { threadId, threadChatId } = await createTestThread({
        db,
        userId: user.id,
        overrides: {
      ...
      await handleAppMentionEvent(
        createBasicEvent({
          text: "<@B123456789> legacy thread follow-up",
          ts: "1234567899.123456",
          thread_ts: "1234567890.123456",
        }),
      );
      ...
      expect(threadChat?.queuedMessages?.[0]?.parts[0]).toEqual(
        expect.objectContaining({
          type: "text",
          text: expect.stringContaining("legacy thread follow-up"),
        }),
      );
    });

    it("drops duplicate Slack delivery follow-ups when the initial message has the delivery marker", async () => {
      const deliveryId = "Ev123";
      const marker = "<!-- terragon-external-task-intake:slack:Ev123 -->";
          ...
          replaceMessages: [
            {
              ...
              model: "sonnet",
              parts: [
                { type: "text", text: "original Slack mention" },
                { type: "text", text: `\n\n${marker}` },
              ],
      ...
      await handleAppMentionEvent(
        createBasicEvent({
          event_id: deliveryId,
          text: "<@B123456789> original Slack mention",
          ts: "1234567899.123456",
          thread_ts: "1234567890.123456",
        }),
      );
      ...
      expect(threadChat?.queuedMessages ?? []).toHaveLength(0);
    });

    it("serializes concurrent deliveries for the same Slack thread key", async () => {
      let createdThreadIds: { threadId: string; threadChatId: string } | null =
        null;
      vi.mocked(newThreadInternal).mockImplementation(async (args) => {
        const created = await createTestThread({
          db,
          userId: args.userId,
          overrides: {
            githubRepoFullName: args.githubRepoFullName,
            repoBaseBranchName: args.baseBranchName ?? "main",
            sourceType: args.sourceType,
            sourceMetadata: args.sourceMetadata,
          },
          chatOverrides: {
            agent: "claudeCode",
            replaceMessages: [args.message],
          },
        });
        createdThreadIds = created;
        return {
          ...created,
          model: args.message.model ?? "sonnet",
        };
      });
      ...
      await Promise.all([
        handleAppMentionEvent(
          createBasicEvent({
            event_id: "Ev-concurrent-1",
            text: "<@B123456789> first concurrent mention",
            ts: "1234567899.123456",
            thread_ts: "1234567890.123456",
          }),
        ),
        handleAppMentionEvent(
          createBasicEvent({
            event_id: "Ev-concurrent-2",
            text: "<@B123456789> second concurrent mention",
            ts: "1234567900.123456",
            thread_ts: "1234567890.123456",
          }),
        ),
      ]);
      ...
      expect(newThreadInternal).toHaveBeenCalledTimes(1);
      expect(createdThreadIds).not.toBeNull();
      const createdThread = await getThreadChat({
        db,
        userId: user.id,
        threadId: createdThreadIds!.threadId,
        threadChatId: createdThreadIds!.threadChatId,
      });
      expect(createdThread?.queuedMessages).toHaveLength(1);
    });
