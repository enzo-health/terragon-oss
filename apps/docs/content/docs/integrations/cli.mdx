...
- **Node.js 20+** or newer
- **npm** package manager (used by the installer for runtime dependencies)
...
<Callout type="info">
  Prefer a manual install instead? Download the latest `terry-cli.tar.gz` asset
  from the [Terragon releases page](https://github.com/terragon-labs/terragon/releases),
  extract it anywhere, run `npm install --omit=dev`, then launch it with
  `node dist/index.js`.
</Callout>
