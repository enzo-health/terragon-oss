...
# Patch claude code when the installed launcher is a text script.
RUN claude_path="$(readlink -f "$(which claude)")" \\
    && if head -c 2 "$claude_path" | grep -q '^#!'; then sed -i.bak -e '1a\\
...
# Create entrypoint wrapper to disable core dumps before starting supervisord
RUN cat > /entrypoint.sh << 'EOF'
#!/bin/bash
set -e
# Disable core dumps to prevent disk pollution in /tmp overlay
ulimit -c 0
