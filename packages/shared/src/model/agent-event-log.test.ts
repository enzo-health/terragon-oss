...
describe("agent-event-log", () => {
  ...
  describe("getActiveAgUiLifecycleAt", () => {
    type AgUiInsertInput = {
      fixture: RunFixture;
      seq: number;
    ...
    it("returns empty result when fromSeq is 0", async () => {
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
      ...
      expect(active).toEqual({ textMessages: [], toolCalls: [] });
    });

    it("reports START-without-END text messages as active", async () => {
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
      ...
      expect(active.textMessages).toEqual([{ messageId: "msg-open" }]);
    });

    it("excludes messages whose START and END are both before the cursor", async () => {
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
      ...
      expect(active.textMessages).toEqual([]);
    });

    it("synthesizes a START for orphan-CONTENT messageIds (legacy pre-bracketing logs)", async () => {
      // Simulates a thread whose event log was written before commit 4e7559a
      // added daemon-delta START/END bracketing. CONTENT rows exist for a
      // messageId that never received a TEXT_MESSAGE_START, and the run is
      // still in-flight (no END either). Without this path, the client would
      // reject CONTENT events on replay for the orphan id.
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
      ...
      expect(active.textMessages).toEqual([{ messageId: "msg-orphan" }]);
    });

    it("does not synthesize a START for orphan-CONTENT that was later closed by an END", async () => {
      // A malformed but terminated history: CONTENT with no START, then END.
      // The client rejects CONTENT regardless and the run is no longer
      // in-flight, so the replay path must not re-open it.
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
    ...
    it("does not duplicate a messageId that has both a real START and earlier orphan CONTENT", async () => {
      // Defensive: if a log somehow shows CONTENT (orphan) then later a
      // proper START for the same id (e.g. a retry wrote the START after
      // the CONTENT landed), the real START wins and the id appears once.
      const fixture = await createRunFixture();
      await insertAgUiRow({
        fixture,
        seq: 1,
      ...
      expect(active.textMessages).toEqual([{ messageId: "msg-reconciled" }]);
    });

    it("merges START-without-END and orphan-CONTENT in a single result", async () => {
      const fixture = await createRunFixture();
      // Classic open message (has START, no END).
      await insertAgUiRow({
        fixture,
        seq: 1,
        eventType: "TEXT_MESSAGE_START",
      ...
      // Orphan CONTENT for a different id.
      await insertAgUiRow({
        fixture,
        seq: 2,
        eventType: "TEXT_MESSAGE_CONTENT",
      ...
      // Closed message — should not appear.
      await insertAgUiRow({
        fixture,
        seq: 3,
        eventType: "TEXT_MESSAGE_START",
