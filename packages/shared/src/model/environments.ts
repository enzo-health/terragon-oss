...
export function applyEnvironmentSnapshotBuildCompletion({
  snapshots,
  snapshot,
  expectedBuildId,
}: {
  ...
  const currentSnapshot = idx >= 0 ? existing[idx]! : null;
  if (
    !currentSnapshot ||
    currentSnapshot.status !== "building" ||
    currentSnapshot.buildId !== expectedBuildId
  ) {
    return {
      applied: false,
      snapshots: existing,
...
export async function completeEnvironmentSnapshotBuild({
  db,
  environmentId,
  userId,
  snapshot,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const existingSnapshots = environment.snapshots ?? [];
    const completion = applyEnvironmentSnapshotBuildCompletion({
    ...
    if (!completion.applied) {
      return {
        applied: false,
        currentSnapshot: completion.currentSnapshot,
      };
    }

    const result = await db
      .update(schema.environment)
      .set({ snapshots: completion.snapshots })
      .where(
        and(
          eq(schema.environment.userId, userId),
          eq(schema.environment.id, environmentId),
          sql`coalesce(${schema.environment.snapshots}, '[]'::jsonb) = ${JSON.stringify(existingSnapshots)}::jsonb`,
        ),
      )
  ...
  const latestEnvironment = await getEnvironment({ db, environmentId, userId });
  const latestSnapshot =
    latestEnvironment?.snapshots?.find(
      (s) => s.provider === snapshot.provider && s.size === snapshot.size,
    ) ?? null;
