...
const DAYTONA_BASE_IMAGE = "ubuntu:24.04";
const SUPERVISORD_COPY_COMMAND =
...
export async function buildRepoSnapshot({
  ...
  let image = buildDaytonaBaseImage();
  ...
  if (setupScript) {
    image = image.runCommands(
      writeBase64FileShellCommand("/tmp/terragon-setup.sh", setupScript),
      writeBase64FileShellCommand(
        "/tmp/terragon-snapshot-run-setup.sh",
        buildSetupRunnerScript(),
      ),
...
export function buildDaytonaBaseImage(): Image {
  const dockerfileCommands = renderDockerfile("daytona")
    .replace(`FROM ${DAYTONA_BASE_IMAGE}`, "")
    .replace(
      SUPERVISORD_COPY_COMMAND,
      writeBase64FileDockerfileCommand(
  ...
  return Image.base(DAYTONA_BASE_IMAGE).dockerfileCommands([
    dockerfileCommands,
  ]);
}

function writeBase64FileDockerfileCommand(
  remotePath: string,
  contents: string,
): string {
  return `RUN ${writeBase64FileShellCommand(remotePath, contents)}`;
}

function writeBase64FileShellCommand(
  remotePath: string,
  contents: string,
): string {
  const encoded = Buffer.from(contents, "utf8").toString("base64");
  return `mkdir -p "$(dirname '${remotePath}')" && printf '%s' '${encoded}' | base64 -d > '${remotePath}'`;
}
