 ...
 *
 * Notes on Test 4:
 *   - If DAYTONA_API_KEY is set, uses the actual Terragon template via snapshot.get().ref
 *     (the production-accurate path). Requires production API key.
...
// ── 3. Daytona: SDK auth via JWT ─────────────────────────────────────────────
async function testDaytonaAuth() {
  console.log("\n── Test 3: Daytona JWT auth + snapshot list ──");
  const daytona = new Daytona({
    jwtToken: DAYTONA_JWT,
    organizationId: DAYTONA_ORG_ID,
  });
...
async function main() {
  ...
  if (!DAYTONA_JWT) {
    console.error("Missing DAYTONA_JWT");
    process.exit(1);
  }
  if (!DAYTONA_ORG_ID) {
    console.error("Missing DAYTONA_ORG_ID");
    process.exit(1);
  }
